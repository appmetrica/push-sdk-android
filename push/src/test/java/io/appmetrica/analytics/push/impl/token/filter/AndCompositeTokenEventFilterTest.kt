...
internal class AndCompositeTokenEventFilterTest : CommonTest() {
    ...
    @Test
    fun shouldSend() {
        val filter = AndCompositeTokenEventFilter(filter1, filter2)
        assertThat(filter.shouldSend(mock())).isTrue()
    }

    @Test
    fun shouldNotSend() {
        val filter = AndCompositeTokenEventFilter(filter1, filter2, filter3)
